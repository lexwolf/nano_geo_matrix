...
inline std::vector<std::pair<double,double>>  nanosphere::cross_sections(int orders, char* mdl , char* mtl ,char* hst ,double omemi, double omema, int omeN, char* sol, double rho, const char* output){
	    int omi;//, ns=0;
// 	    double eps_b, eps_s, eps3=0., 
	    ...
	    crsc.open(out_base / "sts" / "cross_sections.dat");
...
inline std::vector<std::pair<double,std::complex<double>>>  nanosphere::steady_state(int order, char* mdl , char* mtl ,char* hst ,double omemi, double omema, int omeN, char* sol, double rho, const char* output){
	    int omi, ns=0;
		...
		egvaname << "ev" << std::setw(2) << std::setfill('0') << order << ".dat";
        std::ostringstream cffcname;
		cffcname << "a" << std::setw(2) << std::setfill('0') << order << ".dat";
        std::ostringstream cffcnbme;
		cffcnbme << "b" << std::setw(2) << std::setfill('0') << order << ".dat";

	std::ofstream egva(out_base / "sts" / "eigenvalues" / egvaname.str());
	std::ofstream cffc(out_base / "sts" / "coefficients" / cffcname.str());
	std::ofstream cffb(out_base / "sts" / "coefficients" / cffcnbme.str());
	std::ofstream comp;
	std::ofstream stat;
	std::ofstream heiv;
	heiv.open(out_base / "sts" / "highest_ev.dat");
	if (order == 1){
	    stat.open(out_base / "sts" / "stationary.dat");
        ...
        comp.open(out_base / "sts" / "compounds.dat");
...
inline std::complex<double> nanosphere::numerical(int order, char* mdl , char* mtl ,char* hst,
								...
								char* sol, double rho, const char* pump_md, bool evolve_N, const char* output){
		...
		fncname << "func" << std::setw(2) << std::setfill('0') << order << ".dat";
		aj_name << "a"    << std::setw(2) << std::setfill('0') << order << ".dat";

		std::ofstream fnct(out_base / "num" / fncname.str());
		std::ofstream a_aj(out_base / "num" / aj_name.str());
		std::ofstream dyna;

		if(order==1){
			dyna.open(out_base / "numtime.dat");
...
inline std::complex<double> nanosphere::analytical(int order, char* mdl , char* mtl ,char* hst,
		...
		std::ofstream fnct(out_base / "anl" / fncname.str());
		std::ofstream a_aj(out_base / "anl" / aj_name.str());
		std::ofstream dyna;

		if(order==1){
			dyna.open(out_base / "anltime.dat");
			dyna<<"# * ANALYTICAL *\n#\n# file generated by ** cupN.H **\n#\n";
			dyna<<"# PARAMETERS:\n";
			dyna<<"# G = "<<pG<<" a = "<<this->a<<"\n";
			dyna<<"# ome = "<<omeeV<<" eV\n";
