...
inline std::vector<std::pair<double,std::complex<double>>>  nanosphere::steady_state(char* mdl , char* mtl ,char* hst ,double omemi, double omema, int omeN, char* sol, double rho, const char* output){
		...
		odv = new std::complex<double> [3];
            ...
            std::ofstream stat(out_base / "stationary.dat");
            if (!stat) {
                std::cerr << "Error: Cannot open output file" << std::endl;
                exit(1);
            }
            std::ofstream comp(out_base / "compounds.dat");
            if (!comp) {
                std::cerr << "Error: Cannot open output file" << std::endl;
                exit(1);
            }
            std::ofstream egva(out_base / "eigenvalues.dat");
            if (!egva) {
                std::cerr << "Error: Cannot open output file" << std::endl;
                exit(1);
            }
...
inline std::complex<double> nanosphere::numerical(char* mdl, char* mtl, char* hst,
    ...
    std::ofstream dyna(out_base / "numtime.dat");
    if (!dyna) { std::cerr << "Error: Cannot open output file\n"; std::exit(1); }

    std::ofstream fnct(out_base / "numfunc.dat");
...
inline std::complex<double> nanosphere::analytical(char* mdl , char* mtl ,char* hst,
		...
		if (T==0){ 
		    ...
		    steste=1;
                ...
                std::ofstream dyna(out_base / "anltime.dat");
                if (!dyna) {
                    std::cerr << "Error: Cannot open output file" << std::endl;
                    exit(1);
                }
                std::ofstream fnct(out_base / "anlfunc.dat");
                if (!fnct) {
                    std::cerr << "Error: Cannot open output file" << std::endl;
                    exit(1);
                }
